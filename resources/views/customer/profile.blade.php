    ...
    <div class="row g-4">
        ...
        <div class="col-12 col-md-5">
            ...
            <div class="card p-3 p-md-4">
                ...
                @if($orders->isEmpty())
                    <p class="text-muted small text-center py-2">No orders yet.</p>
                @else
                    ...
                    @endforeach
                    <a href="{{ route('customer.orders') }}" class="btn btn-outline-secondary btn-sm w-100 mt-2">View All
                        Orders</a>
                @endif
            </div>
        </div>
            ...
            <div class="card p-3 p-md-4">
                ...
                @if($addresses->isNotEmpty())
                    @foreach($addresses as $addr)
                        <div class="border rounded p-3 mb-2 {{ $addr->is_default ? 'border-warning bg-light' : '' }}">
                                ...
                                <div class="d-flex gap-1 flex-shrink-0">
                                    @if(!$addr->is_default)
                                        <form method="POST" action="{{ route('customer.address.default', $addr->id) }}">
                                            @csrf
                                            <button class="btn btn-sm btn-outline-warning">Default</button>
                                        </form>
                                    @endif
                                    <form method="POST" action="{{ route('customer.address.delete', $addr->id) }}"
                ...
                <form method="POST" action="{{ route('customer.address.store') }}">
                    ...
                    <div class="row g-2">
                        ...
                        <div class="col-12 col-sm-8">
                            <input type="text" name="address_line" class="form-control" placeholder="Full address" required>
                        </div>
